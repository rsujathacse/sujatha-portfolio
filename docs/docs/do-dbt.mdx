...
**Note:** Why 512 MB RAM?
The $4/mo Droplet has 512 MB of RAM, which is tight for Python package installation. You'll add a swap file in Part 2 to give the system headroom during the dbt install. It keeps costs low and shows a real-world constraint that developers might encounter on budget.
...
**Note:** Use the VPC network hostname, not the public one
...
![memory image](/img/dbtdo/3.memory.png)
...
![mkswap output image](/img/dbtdo/4.memoryswap.png)
...
![Terminal output showing Swap image](/img/dbtdo/5.swapmemoryfree.png)
...
![Terminal output with python version image](/img/dbtdo/6.pythonversion.png)
...
![Terminal output showing dbt Core installed image](/img/dbtdo/7.dbt-version-instllation.png)
...
`jaffle_shop` is the canonical dbt sample project name, used across the dbt community and official documentation.
...
![Terminal showing dbt init jaffle_shop with all configuration inputs image](/img/dbtdo/9.dbt-installation-complete.png)

After you enter these values, dbt automatically runs `dbt debug` to validate the connection before creating the project.

![Terminal showing Connection test: OK image](/img/dbtdo/8.dbtinit.png)
...
![Terminal showing dbt run output image](/img/dbtdo/10.dbt-completion-successful.png)
...
![dbt docs overview page image](/img/dbtdo/11.dbt-docs-overview.png)
...
![my_first_dbt_model detail page image](/img/dbtdo/12.my-first-dbt-model-table.png)
...
![my_second_dbt_model detail page image](/img/dbtdo/13.my-second-dbt-model.png)
...
![dbt lineage graph image](/img/dbtdo/14.dbt-dag.png)
